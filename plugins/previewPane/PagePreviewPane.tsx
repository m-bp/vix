...
export default function PagePreviewPane(props: Props) {
  const { previewSecretId, apiVersion, locale } = props
  ...
  if (!slug) {
    return (
      <Card tone="primary" margin={5} padding={6}>
        <Text align="center">Please add a slug to see the preview!</Text>
      </Card>
    )
  }
...
const Iframe = memo(function Iframe(
  ...
  const { apiVersion, previewSecretId, slug, locale } = props
